...
export function GoalCard({ goal, currentBalance, onDelete }: Props) {
  ...
  const progress = goal.targetCents > 0
    ? Math.min(1, Math.max(0, currentBalance / goal.targetCents))
    : 1;
  ...
  return (
    <View style={[styles.card, cardShadow, { backgroundColor: colors.card, borderColor: completed ? colors.success : colors.border }]}>
      <View style={styles.header}>
        <View style={[styles.iconBox, { backgroundColor: completed ? colors.successBackground : colors.rewardBackground }]}>
...
const styles = StyleSheet.create({
  card: {
    padding: layout.spacing.card, borderRadius: layout.radius.card,
  ...
  amounts: { flexDirection: 'row', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' },
